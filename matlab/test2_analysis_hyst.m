...
function test2_analysis(filename, start_time, end_time)
	...
	text(time(1) + 0.55 * (time(end) - time(1)), setpoint2 + 0.5, sprintf('Limite di temperatura superiore: %.2f °C', setpoint2 + 0.5), 'Color', col_band, 'VerticalAlignment', 'top', 'HorizontalAlignment', 'center');
	yline(setpoint2 - 0.5, 'Color', col_band, 'LineStyle', ':', 'LineWidth', 1.8, 'HandleVisibility', 'off');
	text(time(1) + 0.55 * (time(end) - time(1)), setpoint2 - 0.5, sprintf('Limite di temperatura inferiore: %.2f °C', setpoint2 - 0.5), 'Color', col_band, 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'center');
	...
	if ~isnan(ss_error_pos)
		...
		text(time(setpoint_change_idx + cross_idx + ss_error_pos_idx - 2) - 0.005 * (time(end) - time(1)), setpoint2 + ss_error_pos, sprintf('Errore massimo positivo a regime: +%.2f °C', ss_error_pos), 'Color', col_err, 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'right');
	end
	...
	if ~isnan(ss_error_neg)
		...
		text(time(setpoint_change_idx + cross_idx + ss_error_neg_idx - 2) + 0.005 * (time(end) - time(1)), setpoint2 + ss_error_neg, sprintf('Errore massimo negativo a regime: -%.2f °C', abs(ss_error_neg)), 'Color', col_err, 'VerticalAlignment', 'top', 'HorizontalAlignment', 'left');
	end
	...
	exportgraphics(gcf, '/Users/giacomo/Desktop/hyst_ppt.pdf', 'ContentType', 'vector');
end
