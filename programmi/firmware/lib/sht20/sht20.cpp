...
sht20::sht20(uint32_t rp) {
  ...
  error = 0U;         // codice di errore del sensore SHT20
}

// inizializzazione del sensore SHT20
bool sht20::begin() {
  // inizializzazione del sensore SHT20 e verifica errori
  if (!sensor.begin()) {
  ...
  switch (state) {
    ...
    case SHT20_WAIT_TEMP:
      ...
      if (sensor.reqTempReady()) {
        ...
        error = 0; // reset codice di errore
        state = SHT20_READY_FOR_RH_REQ; // aggiornamento stato
      }
      return false; // nessun nuovo dato disponibile
    ...
    case SHT20_WAIT_RH:
      ...
      if (sensor.reqHumReady()) {
        ...
        error = 0; // reset codice di errore
        state = SHT20_NEW_DATA; // aggiornamento stato
      }
      return false; // nessun nuovo dato disponibile
