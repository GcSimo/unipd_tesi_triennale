...
#define SP_UPDATE_DELAY 500      // ritardo di aggiornamento del setpoint
...
#define ERR_SHT20_TIMER 1000     // durata accensione led per errori sht20
#define ERR_OVERFLOW_TIMER 1000  // durata accensione led per errori di overflow
...
#define TEMP_PID_WINDUP 0  // 0 = no windup | 1 = clamping | 2 = back calculation
...
#define CTRL_MIN_OUTPUT 0.0f   // valore minimo di uscita del PID
#define CTRL_MAX_OUTPUT 100.0f // valore massimo di uscita del PID
#define CTRL_PWM_PERIOD 8000   // periodo del pwm per controllo attuatori (in ms) (max suggested 30000 ms)
...
#define SW_OFF LOW     // switch basso
#define SW_ON HIGH     // switch alto

// modalità di controllo della temperatura e dell'umidità
#define AUTOM_CTRL 0   // controllo automatico tramite PID o isteresi
...
struct status {
  ...
  uint16_t temp_sht20;      // temperatura rilevata dal sensore SHT20
  uint16_t rh_sht20;        // umidità rilevata dal sensore SHT20
  uint16_t temp_setpoint;  // setpoint temperatura
  uint16_t rh_setpoint;    // setpoint umidità

  // valori dei potenziometri
  uint16_t temp_pot_value; // valore potenziometro temperatura
  uint16_t rh_pot_value;   // valore potenziometro umidità
...
struct pid {
  ...
  int16_t sum1;   // somma degli errori tra setpoint e valore misurato
  uint16_t sum2;  // somma dei valori misurati
  uint16_t sum3;  // somma dei prodotti tra indice e valore misurato
