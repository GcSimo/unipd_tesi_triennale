...
void update_temporized_errors() {
  for (uint8_t i = 10; i < 16; i++) {
    if (err_check(i) && millis() - err_timers[i] >= ERR_TIMER) {
      err_rm(i); // rimuove l'errore temporizzato
      err_timers[i] = 0; // resetta il timer
    }
  }
}
...
void check_temp_range() {
  ...
  if (!err_check(ERR_LOW_TEMP) && status.temp_sht20 < status.temp_setpoint - TEMP_ERR_THLD)
    err_set(ERR_LOW_TEMP); // errore temperatura troppo bassa
  else if (err_check(ERR_LOW_TEMP) && status.temp_sht20 >= status.temp_setpoint - TEMP_ERR_THLD)
    err_rm(ERR_LOW_TEMP); // rimozione errore temperatura troppo bassa

  // temperatura troppo alta
  else if (!err_check(ERR_HIGH_TEMP) && status.temp_sht20 > status.temp_setpoint + TEMP_ERR_THLD)
    err_set(ERR_HIGH_TEMP); // errore temperatura troppo alta
  else if (err_check(ERR_HIGH_TEMP) && status.temp_sht20 <= status.temp_setpoint + TEMP_ERR_THLD)
    err_rm(ERR_HIGH_TEMP); // rimozione errore temperatura troppo alta
}

// verifica se l'umidità misurata rientra nel range di sicurezza
void check_rh_range() {
  // umidità troppo bassa
  if (!err_check(ERR_LOW_RH) && status.rh_sht20 < status.rh_setpoint - RH_ERR_THLD)
    err_set(ERR_LOW_RH); // errore umidità troppo bassa
  else if (err_check(ERR_LOW_RH) && status.rh_sht20 >= status.rh_setpoint - RH_ERR_THLD)
    err_rm(ERR_LOW_RH); // rimozione errore umidità troppo bassa

  // umidità troppo alta
  else if (!err_check(ERR_HIGH_RH) && status.rh_sht20 > status.rh_setpoint + RH_ERR_THLD)
    err_set(ERR_HIGH_RH); // errore umidità troppo alta
  else if (err_check(ERR_HIGH_RH) && status.rh_sht20 <= status.rh_setpoint + RH_ERR_THLD)
    err_rm(ERR_HIGH_RH); // rimozione errore umidità troppo alta
}